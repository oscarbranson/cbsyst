...
def GLODAPv2_comparison(figdir='.'):
    ...
    cpH = cb.Csys(TA=gd.talk, DIC=gd.tco2, T=gd.temperature, S=gd.salinity, P=gd.pressure, TP=gd.phosphate, TSi=gd.silicate)
    print('   Making plots...')
    ...
    cTA = cb.Csys(pH=gd.phtsinsitutp, DIC=gd.tco2, T=gd.temperature, S=gd.salinity, P=gd.pressure, TP=gd.phosphate, TSi=gd.silicate)
    print('   Making plots...')
    ...
    cDIC = cb.Csys(pH=gd.phtsinsitutp, TA=gd.talk, T=gd.temperature, S=gd.salinity, P=gd.pressure, TP=gd.phosphate, TSi=gd.silicate)
    print('   Making plots...')
...
# plotting function
def cplot(obs, pred, var, cvar, c, alpha=0.4, pclims=[.05, 99.95]):
    fig = plt.figure(figsize=[11, 3.5])

    ax1 = fig.add_axes([.07, .13, .33, .8])
    ax2 = fig.add_axes([.47, .13, .33, .8])
    hax = fig.add_axes([.805, .13, .08, .8])
    cax = fig.add_axes([.9, .13, .015, .8])
    ...
    ax2.axhline(0, ls='dashed', c=(.5, .5, .5, .8), zorder=1)
    ...
    hax.hist(diff, bins=bins, orientation='horizontal', color=(.6, .6, .6))
    ...
    hax.set_ylim(ax2.get_ylim())
    hax.axhline(0, ls='dashed', c=(.5, .5, .5, .8), zorder=1)
