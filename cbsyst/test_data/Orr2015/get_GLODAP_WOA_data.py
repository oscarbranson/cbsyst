...
def get_GLODAP_WOA(save=True):
    ...
    if not os.path.exists('./GLODAP_data/'):
        ...
        for f in GLODAP_files:
            with TqdmUpTo(unit='B', unit_scale=True, miniters=1,
                          desc='GLODAP {:}'.format(f)) as t:
                ureq.urlretrieve(GLODAP_url.format(f),
    ...
    for f in os.listdir('GLODAP_data/'):
        ...
        tmp = np.genfromtxt(tar.extractfile(var + '/' + var + '.data'),
                            delimiter=',')
        # reshape to 3D array
        glodap[var.lower()] = np.reshape(tmp, (33, 360, 180))

    # axes
    lat = np.genfromtxt(tar.extractfile(var + '/' + 'Lat.centers'),
                        delimiter=',')
    lon = np.genfromtxt(tar.extractfile(var + '/' + 'Long.centers'),
                        delimiter=',')
    dep = np.genfromtxt(tar.extractfile(var + '/' + 'Depth.centers'),
                        delimiter=',')
    ...
    for p in params:
        ...
        for f in tar.getnames():
            dat.append(pd.read_csv(tar.extractfile(f),
                                   names=columns,
                                   comment='#'))
        dat = pd.concat(dat)
        dat.loc[:, 'var'] = p
    ...
    interp = {}
    for p in params:
        interp[p] = RegularGridInterpolator((woa_lat, woa_lon, woa_dep),
                                            gwoa[p])
    ...
    glodap_woa = pd.DataFrame({'lat': glat.ravel(),
                               'lon': glon.ravel(),
                               'dep': gdep.ravel()})
    ...
    for p in params:
        glodap_woa.loc[:, p] = interp[p](glodap_woa.loc[:, ['lat', 'lon', 'dep']].values)
